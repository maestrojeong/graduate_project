...
def data_extractor(name, start,end, minimum_drug_sequence = 5, delay = 10):
    '''
    input:
...
def recurrent_neural_network(x):
    ...
    temp_outputs = tf.transpose(tf.stack(outputs[delay:]),[1,0,2])
...
train_data = data_extractor(file_name, 1,train_index, minimum_drug_sequence, delay)
test_data = data_extractor(file_name, train_index+1, int(train_index*5/4), minimum_drug_sequence, delay)

print_spec(total_drugs, year, train_data, test_data)

#Training model
x = tf.placeholder('float', [None, parser_size, n_classes])
y = tf.placeholder('float', [None, parser_size - delay, n_classes])
...
for i in range(len(prediction)):
    for j in range(parser_size - delay):
        for k in range(total_drugs+1):
            if test_data['output'][i][j][k]==1:
                total+=1
                rank = 0
...
f = open('rnn_model2_{}_{}_{}.txt'.format(year, epoch,delay),'w')
